...
class AudioPipeline:
    ...
    async def process_chunk(self, chunk: AudioChunk) -> None:
        ...
        # Automatically finalize the conversation if it has been active for 10 seconds
        # to trigger a summary update of only the necessary stuff.
        if session_id in self._conversations:
            active_state = self._conversations[session_id]
            if active_state.has_speech:
                conv_duration = now - active_state.started_ts
                if conv_duration >= 10.0:
                    logger.info(
                        "Session %s conversation %s reached 10 seconds active duration, finalizing for intermediate summary update",
                        session_id,
                        active_state.conversation_id,
                    )
                    await self._finalize_conversation(session_id, "10s duration limit")
