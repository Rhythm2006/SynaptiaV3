...
async def stream_conversation():
    """SSE endpoint providing conversation events for the inference service."""
    logger.info("📡 Inference service connected to /stream/conversation")
    import random
    import json

    mock_people = ["person_001", "person_002", "person_003"]

    async def event_generator():
        try:
            while True:
                # 1. Send PERSON_DETECTED event
                person_id = random.choice(mock_people)
                person_event = {
                    "event_type": "PERSON_DETECTED",
                    "person_id": person_id,
                    "timestamp": datetime.utcnow().isoformat() + "Z"
                }
                yield f"event: conversation\ndata: {json.dumps(person_event)}\n\n"

                # Wait before conversation end
                await asyncio.sleep(12.0)

                # 2. Send CONVERSATION_END event
                conv_end_event = {
                    "event_type": "CONVERSATION_END",
                    "person_id": person_id,
                    "timestamp": datetime.utcnow().isoformat() + "Z",
                    "conversation": [
                        {"speaker": person_id, "text": "Hello! How are you doing today?"},
                        {"speaker": "patient", "text": "I am doing well, thank you."},
                        {"speaker": person_id, "text": "It was wonderful to visit and see you."}
                    ]
                }
                yield f"event: conversation\ndata: {json.dumps(conv_end_event)}\n\n"

                await asyncio.sleep(8.0)
        except asyncio.CancelledError:
            logger.info("📡 /stream/conversation client disconnected")
