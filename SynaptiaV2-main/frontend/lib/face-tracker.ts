...
export class FaceTracker {
  ...
  update(detections: Detection[]): DetectedFace[] {
    ...
    const nextTrackedFaces = new Map<string, TrackedFace>()
    ...
    for (const [id, trackedFace] of this.trackedFaces) {
      ...
      for (let index = 0; index < unmatchedDetections.length; index += 1) {
        const detection = unmatchedDetections[index]
      ...
      if (bestMatch) {
        ...
        currentFaces.set(id, updatedFace)
        nextTrackedFaces.set(id, updatedFace)
        // Remove matched detection
        unmatchedDetections.splice(bestMatch.index, 1)
      } else if (trackedFace.missedFrames + 1 <= this.maxMissedFrames) {
        // Keep the track briefly when a frame is blurred or the detector drops
        // a result. It is not returned as an active face until matched again.
        nextTrackedFaces.set(id, {
          ...trackedFace,
          missedFrames: trackedFace.missedFrames + 1,
        })
      }
    }
    ...
    unmatchedDetections.forEach((detection) => {
      ...
      currentFaces.set(id, newFace)
      nextTrackedFaces.set(id, newFace)
    })

    // Update tracked faces
    this.trackedFaces = nextTrackedFaces
